...
interface FormL1ToL2BridgeProps extends HTMLAttributes<HTMLDivElement> {
  localToken: Address
  remoteToken: Address
  tokenId: string
  name: string
...
export function FormL1ToL2Bridge({
  localToken,
  remoteToken,
  tokenId,
  name,
  ...
  const simulateErc721Approve = useSimulateErc721Approve({
    address: localToken,
    args: [l1ERC721BridgeAddress, BigInt(tokenId)],
  })
  ...
  const simulateErc721Bridge = useSimulateContract({
    ...
    query: {
      enabled: readErc721GetApproved.data === l1ERC721BridgeAddress,
    },
  })
